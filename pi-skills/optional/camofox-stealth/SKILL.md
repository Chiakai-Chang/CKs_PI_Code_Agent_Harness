...
- **Windows / macOS（重要）**：本後端在非 Linux 一律 `headless: true`（見 `server.js`：xvfb 僅 Linux 建立），**noVNC 不會起、也不會開可見視窗**。所以 **VNC 交接在 Windows 不適用**。改用 **`/weblogin <domain>` 命令**（stealth-web-bridge 提供；命名為 `/weblogin` 以避開 Pi 內建的 `/login`）：
