---
name: hello-reflect
description: 自動化反思系統。在 Session 結束時自動掃描對話紀錄，擷取使用者的修正建議與偏好，並協助將其轉化為持久的專案規範（如 CLAUDE.md、GEMINI.md、.cursorrules、.windsurfrules、.aider.instructions.md）。
---
...
*   **規範寫入**：自動更新專案根目錄的規範文件（依當前 Agent 工具支援 `CLAUDE.md`, `GEMINI.md`, `.cursorrules`, `.windsurfrules`, `.aider.instructions.md`）。
...
3.  **語義總結與同步**：
    *   讀取現有的規範文件（依當前執行的 Agent 選擇 `CLAUDE.md`、`GEMINI.md`、`.cursorrules` 等）。
    *   將新擷取的學習點與現有規則合併。
...
4.  **用戶確認**：
    *   顯示預計更新的規範文件內容。
    *   詢問使用者：「是否確認更新開發規範？（例如更新 CLAUDE.md、GEMINI.md 或 .cursorrules 等，並建議同步更新至 LLM Wiki 以利長期沉澱）」
