...
| cua | reference-clone | research/cua | 已審視(窄範圍:沙箱與權限模型) | docs/prior-art/2026-08-10-cua-review.md | **已採用(概念)**:deny-by-default 的極性 —— 把無界的那一半(有哪些寫檔方式)換成有界的白名單(哪些指令是唯讀);**待評估**:統一授權點(所有守衛經過同一個判斷,Round 12 已量到它們互相搶話) | VM/容器沙箱(lume、kasm:使用者要 agent 改自己的專案,整台隔離等於什麼都不能做)、Rego/YAML 政策檔與 managed+user 雙層(多租戶用的,我們單機單使用者) | 2026-08-10 |
| ultimate-pi | reference-clone | research/ultimate-pi | 已審視(實作層) | docs/prior-art/2026-08-09-ultimate-pi-review.md | **待移植**:`Steer` 階段 —— 拒絕之後的修補必須有界(只修被核准的缺口,不擴大範圍);**待評估**:風險分級決定儀式深度(深度可調、流程不可省),但要先量哪些儀式對小任務是浪費 | 整套 `/harness-*` 指令族(我們的介面是 bridge 與擋阻,且擁有者要的是不用記得觸發)、`plan-packet.yaml` 取代 `planning.md`(與 C.A.S.E. 打架、收益為零)、Sentrux steward 與 graph bootstrap(依賴我們沒有的圖層) | 2026-08-09 |
