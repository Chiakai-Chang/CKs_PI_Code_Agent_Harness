...
- [ ] **環境假設稽核**：**原專案對它的執行環境假設了什麼，而那在這裡不成立？**
      逐項寫下來，不要只想「功能有沒有搬過來」。2026-07-30 的實地驗證中，
      七個真缺陷有五個落在同一個 bridge 上，而每一個都是一條沒被搬過來的假設：
      子代理會有節制地結束（實際在步驟交界靜默結束）、
      子代理的輸出有界（實際撐爆 V8 字串上限，**殺死母行程**）、
      子代理不會亂改東西（實際改了不相關的原始碼，且主 log 看不到）、
      失敗訊息有意義（實際只有啟動橫幅）、
      多代理扇出划算（本機 18 t/s 下 44 分鐘零產出）。
      **這才是「蒸餾過頭」的實際意義——不是搬太多東西，是把環境假設留在了原專案裡。**
      至少要問：緩衝區有沒有上限？子行程的權限有沒有邊界？
      成本模型在本機吞吐下還成不成立？失敗路徑說不說得出原因？
...
### 10. 嚴禁辯解與表面敷衍 (Anti-Defensiveness & Deep Verification)
*   **歷史痛點**：在面對使用者質疑「功能是否真正完整蒸餾」時，AI 代理曾陷入浮躁心理，未先徹底清查研究資產（如 `research/pi-browser-harness/skills/deep-research`），即急於用既有的表面文字與辯解回應使用者，試圖掩蓋缺漏。
*   **進化規則**：
    1. **無深度清查不作答**：當使用者質疑功能缺失或考驗蒸餾完整度時，**第一動作必須是親自讀取並審計 `research/` 原始倉庫與程式碼**，絕不憑印象或既有文檔直接辯解。
    2. **承認缺漏並實體補齊**：若發現確實遺漏了核心演算法或技能（如 `deep-research`），必須開誠佈公承認，並立刻建立實體 Core Skill、補齊 Restore/Uninstall 維護鎖與寫入測試，而非在對話中進行文字遊戲。
    3. **證據高於言語**：始終以 `git commit`、實體代碼與 100% 通過的單元測試作為驗證與報告依據。
