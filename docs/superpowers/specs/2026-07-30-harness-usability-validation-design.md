...
| **答案正確性** | **由人獨立重做一次，不採信 agent 的回報** |

最後兩欄都是 2026-07-30 補的，各自因為漏掉它而差點放過一個缺陷。

**副作用**：原本的觀察表只記錄 agent **說了什麼、呼叫了什麼**，沒有記錄它**改變了什麼**——
而 `deep_research` 的子代理是獨立行程、帶 `--no-session`，它們的動作在主 session log 裡
完全不存在。最嚴重的 P5 是被一次順手的 `git status` 抓到的，不是被協定抓到的。
每個 session 結束後必須跑 `git status`，並比對非預期的新增／修改檔案。

**答案正確性**：一個 session 可以完成、格式工整、零守衛觸發，答案卻是錯的。
T4 第三次產出一份漂亮的表，宣告「已盤點完畢、共有兩個」，真實答案是 10 個——
它第一步只 glob 了 `run-*` 與 `launch-*`。**「完成」不是「正確」。**
判斷成敗必須由人**獨立重做一次**，不採信 agent 的回報；
本輪驗證者自己也連錯兩次才做對，正說明容易錯的任務更需要獨立複核。
