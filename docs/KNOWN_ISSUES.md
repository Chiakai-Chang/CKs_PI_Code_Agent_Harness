...
| # | 項目 | 狀態 |
|---|---|---|
| 1 | **被擋下的變更被宣稱為已完成** | 已加守衛(`blocked-claim.ts`)。**分類路徑實測有跑、糾正路徑只有單元測試** —— 實測那一輪模型剛好誠實回報,守衛正確沒誤開火,但沒觀察到它真的發出糾正 |
| 2 | 深度閘、產出閘 | ~~仍未實測~~ **已實測開火並改變行為**(2026-08-06):深度閘在第 9 次呼叫開火、模型開了 3 頁;產出閘開火 2 次、模型寫出 `findings.md`。見 [docs/measurements/2026-08-06-depth-and-artifact-gates-live.md](measurements/2026-08-06-depth-and-artifact-gates-live.md) |
| 3 | `/case` 斜線展開 | `--print` 不展開(對照組 `/browse` 同樣不展開),只能在 TUI 驗 |
| 4 | 結論隨施壓翻轉 | 刻意不做成閘,理由見上一節 |
| 5 | 編造搜尋端點網址 | 緩解文字已加,未量測 |
| 6 | bash 圍堵是形狀比對 | 擋重導向 / `tee` / `cp`·`mv` / `mkdir`,解析不了就放行。**不是沙箱**,已明說 |
| 7 | 19 個外來技能佔著描述層 | 來自 `~/.agents/skills/`,本 harness 的 tiering 管不到 |
| 8 | 120 個 catalog 技能未逐一評估 | 全部落檔於 `docs/measurements/skill-reachability.md` |
| 9 | C.A.S.E. 範例任務不符自己協定 | 5 個 reference 任務全部 `DONE` 但無 `retro.md`。已回報上游,**不替別人已完成的工作捏造復盤** |

### 一個方法論教訓

第 2 項曾被我寫成「探針結構上驗不到」。**那句話只對那支探針成立。** 使用者質疑之後,我自己建了一個真實 C.A.S.E. 專案跑,二十分鐘內找出三個真缺陷,其中一個是安全相關的圍堵繞過(`bash` 可以繞過只掛在 `write`/`edit` 的目錄圍堵守衛)。
