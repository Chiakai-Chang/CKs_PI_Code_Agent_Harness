...
## 十、第五輪：根因修好之後，判準才看得見真正的問題

修掉 `readability.ts` 剝除網址之後：

| 訊號 | 修前 | 修後 |
|---|---|---|
| 搜尋結果中的網址 | 632 次搜尋 = **0** | **278** 條 |
| 死迴圈 | 598 次搜尋、25 分鐘逾時 | 未再出現 |
| search/open 比值 | 0.01 | 0.38–1.25 |

但量測回報 3/5，而用同一支 `judge` 對同樣五份 session 重評是 **0/5**。

### 又一個代理物：stdout 串流 vs session 檔

`run_once` 從 `pi --print --mode json` 的 **stdout** 收集資料，那份串流比 session 檔少
——少掉的正是**寫進檔案的引用**。於是 live 端只看到少數幾個「都開過」的網址就放行，
放過了三次分別捏造 5、5、13 個引用的執行。

已改為從 **session 檔**評分。live 分數與事後重評終於是同一個數字——
**基線要有意義，這是前提**。
...
> If search results ever carry URLs again, revisit this — citing a listed search
> result without opening it would then be weak sourcing rather than invention.

現在正是那個時候。同樣五次執行：**引用 37 個、沒開過 24 個——但其中 20 個是模型在
搜尋結果裡真的看過的位址**，只有 4 個真正憑空。

**URL 修法把「捏造」從幾乎全部降到 4/37（11%），而我的判準把這個進步藏起來了。**

於是判準拆成兩種不同的失敗：

* **誠實**（絕對底線，直接失敗）：這個位址在整個 session 裡出現過嗎？
* **嚴謹**（門檻）：`min_sources` 只數**真的開過**的頁面；「列出但沒開」如實回報、不致命

### 憑空的那幾個有共同形狀

```
https://biggo.com.tw/s/智慧攝影+門鈴/
https://www.momoshop.com.tw/search/智慧門鈴
```

**全是自己組出來的「站內搜尋網址」**——套用已知網站的搜尋路徑格式，不是隨機幻覺。
這是可辨識的模式，也是下一輪要處理的具體對象。

### 這一輪的方法收穫

1. **修好根因之後，要回頭檢查判準是不是還在用舊世界的假設。** 我預測到了這件事並寫進
   註解，但仍然是量測結果逼我回去讀那句話的。**寫下預測不等於會記得回來看。**
2. **代理物的第四種型態：串流代替紀錄。** 前三次是「新聞首頁代替搜尋結果頁」「單元測試
   代替送達」「摘要代替現場」。這次是 stdout 代替 session 檔。
3. **一個好的判準要能分辨失敗的種類，不只給分數。** 現在五次執行有四種不同的失敗訊息，
   每一種指向不同的修法；先前它們全被記成「0 sources」。
