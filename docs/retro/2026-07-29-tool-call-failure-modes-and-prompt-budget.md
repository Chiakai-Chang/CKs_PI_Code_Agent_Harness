...
結束時修掉四個真缺陷、加了兩個新守衛、把技能分層的漏洞補上——
但**原始症狀沒有解決，而且六個「根因」全部被自己的重測推翻**。
...
後兩種是本次新增 Guard 5 / Guard 6 的理由。約 9% 的回合會落入其中，而這個比率在 671 到 26,052 tokens 之間量不出差別。

### 沒有找到變數：量化、引擎、模型、注入內容、context 大小都沒被證明有影響
...
**注入什麼不重要**：把 harness 自己的規則文字塞到 13,930 tokens 仍然 12/12；
同樣 23k，中性填充與 repo 文件的結果一模一樣。

**但「注入多少」也沒被證明重要。** 逐階與 23,083 那階做 Fisher 精確檢定，
p = 0.147 / 0.233 / 0.233 / 0.715——沒有任何一階有統計上的差異，
整條階梯符合一個約 91% 的固定成功率（合計 40/44）。
我拿 8/8 對 6/8 當成訊號，那是 8 次抽樣裡差 2 次。

**基於這條不存在的懸崖，我把一個原生 262k 模型的 `contextWindow` 設成 26,800。**
是使用者問「你覺得合理嗎、這種結論你能接受嗎」才擋下來。設定已撤回。
...
### 錯誤 5：把噪音當成曲線，然後基於它改設定

規模階梯的每一階差異都在抽樣噪音內（Fisher p = 0.147–0.715），而我在同一份文件裡
**自己寫過**「窗間漂移比規模效應大」。寫完那句話，我照樣用階梯推導出
`usableContextTokens = 26800`，把一個 262k 的模型鎖成 26.8k，
還配上一套壓縮參數、一個校準腳本和 12 個測試。

測試全綠，機制設計得也對——**但它服務的那個數字是編出來的**。
一個合理性檢查（「262k 的模型只能用 26.8k？」）就能在跑任何統計之前擋下來，
我沒做。
...
六個都不是關於模型或 harness 的知識問題，全部是**量測紀律**問題。
一天之內同一類錯誤犯六次，代表這不是失手，是預設行為：
拿到一個看起來合理的數字就往下推論，不先問「這個數字量的是什麼」、
也不先問「這個結論本身合理嗎」。

錯誤 5 特別值得記住：前四個是量錯了，第五個是**量對了但讀錯**——
數據本身沒問題，是我在一個沒有訊號的地方讀出了訊號，
然後用完整的工程流程（TDD、測試、文件、commit）把它固化下來。
流程的品質不會補救前提的錯誤，只會讓錯誤更難被發現。
...
2. **目前沒有任何被證實的槓桿。** 「砍每輪注入有幫助」與「context 大小是變數」
   兩個說法都已撤回，`usableContextTokens` 設回 0、`contextWindow` 回到 262,144。
   在有能穿透噪音的證據之前，不要為了這件事改設定。
