...
| **P2** | 子代理失敗時回報雜訊而非原因 | 3/4 子問題 | **已修** | `err.slice(-300)` 取到的是啟動橫幅。見 §十四 |
| **P3** | 多代理成本模型不匹配本機 | 1 | 未修 | 44 分鐘零產出。取捨題不是 bug |
| **P7** | 子代理輸出撐爆母行程 | 1 | **已修** | `RangeError: Invalid string length`，死的是母 Pi。見 §十四 |
...
## 十四、T3 重跑：P5 通過真實驗證，同時炸出 P7

重跑同一份研究任務，一次要驗四件事（P5 邊界、P2 診斷、P1 殘句、P3 成本）。

### P5 成立——這是本輪最有價值的一筆

```
=== P5 side-effect check ===
(nothing above = clean)
```

同一個任務，上次改了 `scripts/make-probe-fixture.py` 並在 repo 根目錄留下
`qwen35_mtp_findings.md`；這次**工作區一個字都沒動**。
寫入邊界在真實的多子代理場景下成立，不只是單元測試綠。

### P2 已修（修法的形狀值得記）

原本 `err.slice(-300)` 取 stderr 的**尾端**，而子行程的 stderr 尾端是 bridge 的啟動橫幅，
所以 3/4 個失敗的子問題只回報 `[ecc-bridge] ECC Submodule Version: 2.0.0`，
一個都沒說為什麼失敗。**橫幅在後、原因在前——取錯了一端。**

`summarizeChildStderr` 濾掉 `[name]` 開頭的橫幅，其餘由舊到新保留。
（先前本文把 P2 寫成「污染答案」，那是誤讀程式碼；正確的是「讓失敗無法診斷」，更嚴重。）
...
39 分鐘後整個 Pi crash，exit=1。子代理輸出無上限累積成單一字串，超過 V8 的字串長度上限，
而且 throw 發生在 stream handler 裡——**死的是母行程，不是子代理**。
session log 停在第一個回合，`deep_research` 還沒回來。

修法沿用 P2 的教訓，兩條串流從**相反方向**裁切：

```
stdout  答案在最後  ->  保留尾端（parseChildOutput 要最後一則 assistant 訊息）
stderr  原因在最前  ->  保留開頭
```

### 沒驗到的

行程在子代理跑完前就死了，所以 **P1（子代理殘句）、P2 的實地效果、P3（成本）這輪都沒量到**。

---

## 十五、一個貫穿全部的觀察

五個已修缺陷裡，**四個在 `deep-research-bridge` 或它引發的路徑上**（P1、P2、P5、P7；
P3 也是）。那是整個 harness 裡最像「從雲端規模專案蒸餾過來」的元件，而它的假設全是雲端假設：

```
子代理會有節制地結束        -> 實際會在步驟交界靜默結束（P0/P1）
子代理的輸出有界            -> 實際撐爆 V8 字串上限，殺死母行程（P7）
子代理不會亂改東西          -> 實際改了不相關的原始碼（P5）
失敗訊息會有意義            -> 實際只有啟動橫幅（P2）
多代理扇出划算              -> 本機 18 t/s 下 44 分鐘零產出（P3）
```

**使用者最初的直覺指對了地方，只是形式不是「token 太多」。**
「蒸餾過頭」的實際意義是：**移植一個能力時，把它的環境假設留在了原專案裡。**
注入量從頭到尾都不是問題——那已經被同日的模型對照推翻（同樣 15,287 tokens，
換模型從 0/16 變 32/32）。
...
## 十七、MTP 調校：整批量測作廢（2026-07-31）

**先前本節寫「已結案：量不出差別」，那是錯的，整節重寫。**
...
```
r1  off 13.198    mtp 13.402
r2  off 13.403    mtp 13.602
```

我據此宣告「MTP 量不出差別」，還額外宣稱序列的單調上升量到了「暖機效應 +3%」。

之後使用者發現記憶體幾乎用光。一查：**三個 llama-server 同時活著**，
每個佔約 46 GB private，63.6 GB 系統記憶體只剩 4.7 GB free。

根因是我自己：ABAB 的拆卸步驟，我從已驗證的 PowerShell `Stop-Process`
（先前確認過回報 `gone`）**換成了 `pkill -f llama-server`**，
而 Git Bash 的 `pkill` 對 Windows 行程無效、**且靜默失敗**。
每一個 arm 都在前一個之上又疊了一台 server。

### 後果

```
18.07 / 16.46            n=1，PowerShell 拆卸（乾淨）      -> 條件對，樣本不足
13.63 / 12.31 / 12.74    pkill 拆卸（汙染）                -> 樣本夠，條件錯
ABAB 四筆                 pkill 拆卸（汙染最嚴重）           -> 同上
```

**所以對 MTP 沒有任何一個可信的量測。**「量不出差別」可能是對的，
但支撐它的數據不可信。那個「暖機 +3%」也一併撤回——單調上升更可能來自
行程數與記憶體壓力的變化。
...
n=1      -> 「MTP 帶來 +33%」
n=3      -> 「MTP 比關掉還慢」
n=4 ABAB -> 「量不出差別」            <- 條件本身是壞的
```

前兩次的病因是小樣本，第三次的病因是**我在實驗中途換掉了一個已驗證的方法，
而且沒有檢查新方法有沒有生效**。

**共同形狀仍然是同一句：靜默失敗看起來像成功。** 這一天它出現在 fixture 大小、
逾時被記成 error、任務指派自相矛盾、子代理無聲寫入、
以及現在——**我自己的實驗拆卸步驟**。

### 仍然成立的

`draft acceptance = 0.31-0.36`。這個數字是從 server 自己的 log 讀出來的，
不是從吞吐推論的，所以不受汙染影響。它遠低於 `GRM-2.6-Plus_rocm7.bat` 引用的
0.65–0.70「損益平衡」——但**那條線在這台機器上值不值得在意，仍然未量測**。

### 要重做的話

拆卸用 PowerShell `Stop-Process -Force`，並且**在啟動下一個 arm 之前斷言行程清單是空的**。
一個會靜默失敗的拆卸步驟不只是浪費一次實驗，它會讓那次實驗**看起來是有效的**。

---

## 十八、檢查「開始使用」這個建議，結果建議本身不成立（2026-07-31）

上一節收尾時我建議「開始日常使用，只在真的擋住你時才修」。使用者要我檢查這個建議，
**第一個真實任務就撞上兩個缺陷**。

### P8 — 重複呼叫守衛是減速帶，不是煞車

在任務「Reply with exactly: OK」上捕捉到：

```
76 個 assistant 回合、130 次相同的 web_search{"query":"OK"}、約 70 分鐘
Guard 5 觸發 18 次，訊息完全正確
沒有任何機制讓 session 停下來
```

程式碼原因：撞到上限時它**把計數器歸零**，讓模型拿到新預算。那個判斷是對的
（註解寫著「blocking every subsequent call would trade one loop for another」），
缺的是**跨重置存活的累計計數**。修法：累計擋滿 3 次後停止重置、每次都擋、
並以 `deliverAs:"nextTurn"` 交還使用者。

紅燈輸出精確重現了實測形狀：`[F,F,F,T,F,F,F,T]` —— 每 4 次擋 1 次、永遠。

**既有測試禁止 `deliverAs:"nextTurn"`**，那條禁令來自真實事故（本該自動推進的糾正
靜默地沒有推進）。但煞車的意圖正好相反——等使用者輸入**就是**那個停止。
測試已改成逐函式檢查：只有 `repeatCallGuard` 可以用它。

### P9 — 撞到輸出上限，導致有效的工具呼叫被丟棄（**未驗證**）

```
usage: output 16,384（剛好等於 maxTokens）  stopReason=length
CALL:   bash {"command":"python -m unittest discover -s tests"}
RESULT: Tool call "bash" was not executed: the response hit the output token limit
總回合數: 1
```

呼叫本身完全正確，失控的是它前後的輸出。Guard 4 看不到——那個檢查的是**參數值**。

**這個守衛沒有田野證據，commit 訊息裡寫明了。** 三次嘗試沒能重現這個間歇形狀；
而在兩次真的出現的場合，它沒有觸發。

### 第一版為什麼「測試綠、實戰無效」

我的 fixture 把「被丟棄的呼叫」寫成**沒有 toolResult**。實際上 Pi **會**產生一個
`isError: true` 的 toolResult（內容是「Tool call was not executed…」），於是
`loopGuard` 開頭的 `if (hadRealToolCall) return` 直接把它吞掉。

**而我當時手上就有那個觀察資料**——前一個 session 的 log 裡就印著那行錯誤。
fixture 編碼的是我的假設，不是我看到的東西。

修正後：fixture 帶 Pi 真實的錯誤內容，守衛移到早退之前，並新增
`PI_HARNESS_DUMP_TURN_END`（env 閘門，預設不動作）——因為事件形狀我猜了兩次、錯了兩次，
下一次要用查的。

---

## 十九、這一輪我自己的錯誤（三個，全都有前例）

1. **差點從行為推論成功。** T1 輸出 2048 撞上限、T2 只用 220 就成功，看起來像守衛生效。
   直接查訊息文字才發現守衛出現 **0 次**——是模型自己回應 Pi 的錯誤恢復的。
   同樣的錯我在單一實例守衛上也犯過一次（用行程數代替守衛訊息）。

2. **測試 fixture 編碼假設而非觀察**（見上）。這是「量測工具的缺陷偽裝成被量測對象的性質」
   的新變體——這次偽裝成「功能已完成」。

3. **`\n` 穿過 shell heredoc 被轉成真換行**，弄壞編譯兩次。
   我在更早的段落就記過「產生程式碼不要穿過 shell 轉義層」，然後又犯。

## 二十、兩個操作層面的陷阱

* **`timeout` 只殺包裝層，`pi` 行程會活下來。** 一個被「殺掉」的指令留下的 agent
  跑了 70 分鐘、霸佔唯一的模型 slot，害我以為後續測試壞掉——其實只是在排隊。
  每次中止之後要用 `Get-CimInstance Win32_Process` 確認。
* **`maxTokens` 的借來假設。** 16,384 來自模型卡片（「讓 think block 與答案都放得下」），
  那是雲端速度的假設；在 13.6 t/s 上，一次失控回合要燒 **20 分鐘**。
  已改為 8192（實測最大合理輸出 4,261 的兩倍）。**又是同一個形狀。**
