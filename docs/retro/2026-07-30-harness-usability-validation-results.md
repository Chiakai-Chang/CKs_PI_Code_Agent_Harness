...
## 十七、MTP 調校已結案：量不出差別（2026-07-31）

交接檔第 5.4 項掛著「MTP acceptance 0.45 低於損益平衡，該調 `--spec-draft-n-max` 或關掉」。
量完了，結論是**不必調，也不必關**。

ABAB、每個 arm 全新啟動 server、4 次完全相同的請求：
...
MTP 兩輪都 +0.20 t/s（約 1.5%），而**整個序列本身單調上升 0.40** —— 漂移比效應大。

### 過程中我連錯兩次，值得記下來

```
n=1   MTP 18.07 vs off 13.63   -> 我寫成「MTP 帶來 +33%」
n=3   MTP 12.31 vs off 13.63   -> 我改口「MTP 比關掉還慢」
n=4 ABAB                        -> 兩個都是雜訊
```

同一個失效模式在同一段對話裡犯了兩次：**小樣本 + 窗間漂移**。
而這個 repo 的清單第 3 條就寫著「換配置一定要在同一時間窗內 ABAB 重測」。
知道規則跟遵守規則是兩回事。

### 兩個借來的數字，一個確定死、一個未定

* **「acceptance 低於 0.65–0.70 就不划算」**（來自 `GRM-2.6-Plus_rocm7.bat`）——
  **不成立**。實測 acceptance 0.31–0.36，吞吐兩邊一樣。那條線在這台機器上沒有意義。
* **「`--spec-draft-p-min` 越高越好」**（研究答案 + MiMo 卡片，兩個獨立來源）——
  n=1 量到反而更慢，但 n=1 正是產生上面兩個錯誤結論的樣本數，
  所以**記為未量測，不是推翻**。

### 副產品：暖機效應被量到了

序列單調上升 0.40 t/s（約 3%），這是 2026-07-29 交接檔記載卻一直沒量化的
「server 暖機狀態」在吞吐上的形式。它不解釋當初那個 0/12 ↔ 6/8 的巨大差異，
但至少確認這個效應存在、且量級足以蓋過 1.5% 的配置差異。

**實務結論：MTP 調校不是槓桿，別再花時間。** 旗標留著是因為它不花錢，不是因為它賺錢。
