...
def missing_from_catalog(catalog, expected_entries) -> list:
    """Names restore.py demoted into the catalogue that the catalogue does not have.

    A demoted skill is not natively registered, so the catalogue is the only way
    the model can find it. When the generated file falls behind the tiering rules
    those skills exist in neither place. Measured 2026-08-04: 18 local skills
    demoted, 0 present, including `hello-reflect` and `thinking-frameworks`.

    Matching is by name alone, following `merge_into_catalog`: an entry already in
    the catalogue wins, because the external tiering resolved that name first.
    """
    skills = catalog.get("skills") if isinstance(catalog, dict) else None
    have = {e.get("name") for e in skills if isinstance(e, dict)} if isinstance(skills, list) else set()
    return [e["name"] for e in expected_entries
            if isinstance(e, dict) and e.get("name") and e["name"] not in have]


def check_skill_catalog_freshness(root: Path) -> int:
    """Compare the generated catalogue against what restore.py would demote.

    Skipped when there is no catalogue: the file is gitignored, so a fresh
    checkout has none, and failing there would be a check that only ever passes
    on the machine that generated it.
    """
    catalog_path = root / "pi-config" / "skill-catalog.json"
    if not catalog_path.exists():
        print("INFO: no skill-catalog.json (generated by restore.py) — freshness check skipped")
        return 0

    try:
        spec = importlib.util.spec_from_file_location("_restore", root / "scripts" / "restore.py")
        restore = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(restore)
    except Exception as exc:
        print(f"WARN: could not load restore.py for the catalogue check: {exc}")
        return 0

    _mode, core = restore.skill_tiers_from_config(str(root / "pi-config" / "harness-config.json"))
    expected = []
    for sub in ("core", "optional"):
        _kept, tail = restore.tier_local_skills(str(root / "pi-skills" / sub), set(core))
        expected.extend(tail)

    catalog = load_json(catalog_path) or {}
    missing = missing_from_catalog(catalog, expected)
    if missing:
        print(f"FAIL: {len(missing)} local skill(s) are neither natively registered nor in the "
              f"catalogue, so nothing can reach them: {', '.join(missing)}")
        print("      Re-run: python scripts/setup.py --mode restore")
        return 1
    return 0
...
def main():
    ...
    errors += check_skill_catalog_freshness(root)
