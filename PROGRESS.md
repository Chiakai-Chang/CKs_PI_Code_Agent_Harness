...
* **Local DoD**:
  ...
  - [ ] **live 觸發:0 次,而且沒有被測到。** 第一個探針停在 **10 次呼叫**,門檻是 12 ——
        **「沒有觸發」與「沒有用」是兩句話**,這個 repo 混淆過
  - [ ] 觸發條件:再累積 ≥5 個 20+ 呼叫的真實 session,用
        `scripts/report-plan-order.py --all` 重量 no-plan 比例
  - [ ] **若它變成第 11 個從未觸發的機制,就刪掉它**,不要留著當裝飾
