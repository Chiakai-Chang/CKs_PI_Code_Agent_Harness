...
| ~~2~~ ✅ | **4b 重測完成** —— 輪制退場**照設計運作**:20 次擋阻跨 4 輪、一輪一個聲音、**認領前 0 次搜尋成功**(修正前是 15 次) | **但它換來更糟的結果**:模型從 20 次相同拒絕學到「這個工具不能用」,認領後**一次都沒再搜**,交付物自陳「未經驗證」。見 [4b 量測](../docs/measurements/2026-08-08-4b-turn-ramp.md) |
| ~~2a~~ ✅ | **階段轉換通知已驗證(2026-08-09)** —— 通知 turn 7 送達逐字可查;認領前成功研究 **0**、認領後 **22**。4b 的代價沒有重演。見 [量測](../docs/measurements/2026-08-09-phase-notice-live.md) | **但迴圈仍未收斂**:本次撞 2400 秒上限、無 `output.md`,且**推進器 0 次注入** —— 研究型 run 裡模型不停下來講話,推進器就開不了口。那是下一項 |
| ~~2c~~ ❌ | **取消:問題不存在** —— 乾淨重測下推進器注入 2 次與 6 次。那三次 0 是我自己洩漏的孤兒行程 + 180 秒門檻造成的污染。**復盤擋下了一個為不存在的問題而做的機制** | 見 [乾淨重測](../docs/measurements/2026-08-09-clean-research-runs.md) |
| **2d** | **迴圈收斂:2/2 研究型 run 走到 `REVIEW`,任務包完整**(對照 2026-08-06 的 0/5) | 這是擁有者需求裡「切分、逐項、復盤、進 queue」四項的實測達成 |
| ~~2e~~ ✅ | **CLAIM 額度採用 8 輪(未被否證)** —— 2/2 走到 `REVIEW`、認領前成功搜尋 **0/0**(各擋 14 次)、認領後 **16 與 20**。事先寫下的三個下注全中,兩條「抓自己」的拒絕條件都沒觸發 | **n=2 只能否證不能證成**,結論寫成未被否證。見 [下注與判定](../docs/measurements/2026-08-09-claim-budget-bet.md) |
| ~~舊 2e~~ | 原始描述:**先規劃只成立 1/2** —— run 2 在認領前成功搜了 24 次(撐過 4 輪退場) | 加大門檻會讓先規劃更可靠,**但 4b 量到拒絕累積會讓模型放棄整個能力**。現在有通知了,副作用可能已拆掉 —— **那是推論**。要驗就加大門檻重跑,且**同時盯認領後的搜尋數** |
| ~~舊 2c~~ | 原始描述:**推進器在研究型 run 開不了口** —— 它只在「有講話、沒呼叫工具」時說話,而研究型 run 一路在呼叫工具(三次 run 皆 0 次注入) | 沒有推進,任務就只會停在 IN_PROGRESS。這是迴圈收斂的最後一塊 |
| ~~舊 2a~~ | 原始描述:**階段轉換通知已做,但沒響過** —— `phase-notice.ts` 單元 15 條、變異乾淨、走實測通道;而唯一一次真實 run **模型沒認領**,通知沒機會觸發。**依本 repo 規矩是未驗證的守衛。** 見 `02_Task_Queue/Task_020` | 需要 ≥2 次「有認領」的 run 才算數 |
| **2b** | **擋阻額度的鍵是 `階段:工具名稱`** —— 模型換個工具就拿到全新的四輪額度,升級文案從頭重來(實測:`web_open` 在第 4 輪拿到「第一次」文案) | 四個研究工具 = 最多 16 輪拒絕。這是「20 次拒絕」印象的來源,也是模型學到「不能用」的量的來源 |
| ~~舊 2a~~ | 原始描述:**階段轉換通知** —— 研究工具真的開放時,要有訊息告訴模型「現在可以搜了」 | 門檻已經對了,缺的是**供給對的路**。擋阻理由裡寫著「認領後全開」,但沒有東西在階段真的改變時說話。推進器已有通道,是最自然的載體。**在這之前不要開預設** |
